...
class NMTModel(nn.Module):
    def __init__(self, args):
        ...
        self.bart_model = M2M100ForConditionalGeneration.from_pretrained(self.model_name)
        
        # contrastive learning
        self.projection = nn.Sequential(nn.Linear(1024, 1024),
                                        nn.ReLU())

        self.tau = self.args.tau
        self.pos_eps = self.args.pos_eps
        self.neg_eps = self.args.neg_eps
    ...
    def forward(self, 
        ...
        encoder = self.bart_model.get_encoder()
        decoder = self.bart_model.get_decoder()
        ...
        lm_logits = self.bart_model.lm_head(sequence_output)
        ...
        if adv:
            ...
            cont_crit = nn.CrossEntropyLoss(ignore_index=-100)
            ...
            final_loss = self.alpha * nll + self.beta * cont_loss
            # final_loss = self.auto_loss(nll, cont_loss)
    ...
    def generate_adv(self, dec_hiddens, lm_labels):
        ...
        lm_logits = self.bart_model.lm_head(dec_hiddens)
        criterion = nn.CrossEntropyLoss(ignore_index=-100)
    ...
    def generate_cont_adv(self, enc_hiddens, enc_mask,
        ...
        perturb_logits = self.bart_model.lm_head(perturb_dec_hidden)
